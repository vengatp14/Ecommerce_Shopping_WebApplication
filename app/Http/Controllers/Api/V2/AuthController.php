...
class AuthController extends Controller
{
    ...
    public function login_by_phone(Request $request)
    {
        ...
        if ($delivery_boy_condition) {
            $user = User::whereIn('user_type', ['delivery_boy'])->where('phone', "+{$request['country_code']}{$request['phone']}")->first();
        } else {
        ...
        if ($user != null) {
            if($request->password != "" || $request->name != ""){
                if($request->otp != "" || $request->otp_verified == true){
                    if($user->verification_code != $request->otp || $request->otp_verified != true && $request->otp_verified != ""){
                        return response()->json(['result' => false,'status' =>'400', 'message' => translate('OTP is not match'), 'user' => null], 200);
                    }else{
                        $user->name = $request->name;
            ...
            }elseif($user->password == "" || $user->name == ""){
                $user->verification_code = 123456;//rand(100000, 999999);
                $user->save();
                // app('App\Http\Controllers\HomeController')->otp($user->verification_code,$request->phone);
                return response()->json(['result' => false, 'status' =>'201', 'verified'=>'false','message' => translate('Please complete your profile'), 'user' => null], 401);
            }
            if (Hash::check($request->password, $user->password)) {
                return $this->loginSuccess($user);
            } elseif($request->password == ""){
                // app('App\Http\Controllers\HomeController')->otp($user->verification_code,$request->phone);
                return response()->json(['result' => true, 'status' =>'200', 'verified_phone'=>'true' ], 200);
            }else {
                return response()->json(['result' => false, 'message' => translate('Unauthorized'), 'user' => null, 'status' =>'201'], 201);
            }
        } else {
            if($request->phone != "" && $request->country_code){
                if (User::where('phone', "+{$request['country_code']}{$request['phone']}")->first() != null) {
                    return response()->json([
                        'result' => false,
                        'message' => translate('Phone number already exists. Please try another number'),
                        'user_id' => 0
                    ], 201);
                }
                $newUser = new User;
                $newUser->phone = "+{$request['country_code']}{$request['phone']}";
                $newUser->email_verified_at = date('Y-m-d Hms');
                $newUser->verification_code = 123456;//rand(100000, 999999);
                if($request->has('referral_code')){
                    $referral_code = $request->has('referral_code');
                    $referred_by_user = User::where('referral_code', $referral_code)->first();
                    if($referred_by_user != null){
                        $newUser->referred_by = $referred_by_user->id;
                    }
                }
                $newUser->save();
                return response()->json(['result' => true,'new_user' => true, 'status' =>'200', 'message' => translate('OTP has been sent to your phone number')], 200);
            }
            //return response()->json(['result' => false, 'message' => translate('User not found'), 'user' => null], 401);
        }
    }
